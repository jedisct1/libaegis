...
#define aegis256x4_COMMITBYTES aegis256x4_KEYBYTES
 ...
 */
int aegis256x4_mac_update(aegis256x4_state *st_, const uint8_t *m, size_t mlen);
 ...
 */
int aegis256x4_mac_final(aegis256x4_state *st_, uint8_t *mac, size_t maclen);
 ...
 */
int aegis256x4_mac_verify(aegis256x4_state *st_, const uint8_t *mac, size_t maclen);
 ...
 */
void aegis256x4_mac_state_clone(aegis256x4_state *dst, const aegis256x4_state *src);

/*
 * Initialize a state for generating a MAC, with key commitment.
 *
 * st_: state to initialize
 * kc: key commitment output buffer (32 bytes)
 * k: key input buffer (32 bytes)
 *
 * - The same key MUST NOT be used both for MAC and encryption.
 * - The nonce is not used in the MAC mode (fixed to zero).
 ...
 *
 * After initialization, the state can be reused to generate multiple MACs by cloning it
 * with `aegis256x4_mac_state_clone()`.
 */
int aegis256x4_mac_init_with_commitment(aegis256x4_state *st_, uint8_t *kc, const uint8_t *k);

/*
 * Initialize a state for verifying a MAC with key commitment.
 *
 * st_: state to initialize
 * kc: key commitment input buffer (32 bytes)
 * k: key input buffer (32 bytes)
 *
 * - The same key MUST NOT be used both for MAC and encryption.
 * - The nonce is not used in the MAC mode (fixed to zero).
 ...
 */
int aegis256x4_mac_init_verify_commitment(aegis256x4_state *st_, const uint8_t *kc,
                                          const uint8_t *k);
